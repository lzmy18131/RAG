...
def _img(
    page: int, summary: str, q: str, ans: str, qtype: str = "feature", diff: str = "medium"
) -> dict:
...
IMAGE_QUESTIONS = [
    ...
    _img(
        ...
        P6,
        "机器人底部安装了哪些部件？",
        "悬崖传感器、地毯识别传感器、万向轮、主轮、边刷、软胶主刷、集尘进风口。",
        qtype="feature",
    ),
    _img(
        7,
        P7,
        "尘盒的滤网安装在上方还是下方？可以水洗吗？",
        "上置，可以水洗。",
        qtype="maintenance",
        diff="easy",
    ),
...
def _eco(
    pages: list[int], q: str, ans: str, ctx: list[str], qtype: str, diff: str = "medium"
) -> dict:
    return {
        "question": q,
        "question_type": qtype,
...
def main() -> None:
    base = json.loads(
        (PROJECT_ROOT / "data" / "eval_dataset" / "golden_100.json").read_text(encoding="utf-8")
    )
