...
def _retry(max_retries=2, sleep_rec=None):
    return RetryPolicy(
        ...
        cap=8.0,
        jitter=False,
        sleep=(sleep_rec or (lambda d: None)),
    )
...
class TestCircuitBreaker:
    def _gateway(self, threshold=2, cooldown=5.0, clock=None, max_retries=0):
        provs = [
            FakeProvider(
                "primary", lambda m, t: (_ for _ in ()).throw(RetryableProviderError("down"))
            )
        ]
        return _gateway(
            provs,
            retry_policy=_retry(max_retries=max_retries),
            circuit_config=CircuitConfig(failure_threshold=threshold, cooldown_seconds=cooldown),
            now=clock,
        )
...
class TestFailover:
    ...
    def test_all_providers_down_graceful(self):
        ...
        gw = _gateway(
            [FakeProvider("a", bad), FakeProvider("b", bad)], retry_policy=_retry(max_retries=0)
        )
        text, raw = gw.chat([])
        assert raw["gateway_fallback"] is True
...
class TestClassification:
    def test_classify_real_openai_exceptions(self):
        ...
        assert (
            _classify_exc(openai.InternalServerError("is", response=resp(500), body=None)) is True
        )
...
class TestFacade:
    ...
    def test_llm_client_explicit_args_isolated(self):
        ...
        with (
            patch("src.infra.gateway.Provider", return_value=fake),
            patch("src.infra.gateway.get_gateway") as mock_shared,
        ):
            from src.infra.llm_client import LLMClient

            text, raw = LLMClient("http://custom", "k", "m").chat(
                [{"role": "user", "content": "x"}]
            )
        assert text == "iso"
